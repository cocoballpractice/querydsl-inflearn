...
public class QuerydslBasicTest {
    ...
    public void startQuerydsl() {
        ...
        QMember m = new QMember("m"); // 별칭을 직접 설정
        QMember m2 = member; // 기본 인스턴스 사용(Qmember.member) -> 이를 static import로 활용할 수도 있음

        Member findMember = queryFactory
                .select(member)
                .from(member)
                .where(member.username.eq("member1")) // Querydsl에서 자동으로 preparedStatement의 파라미터 바인딩 방식 사용
                .fetchOne();
    ...
    // 정렬 (회원 나이 내림차순 desc, 회원 이름 올림차순 asc, 회원 이름이 없을 시 마지막에 출력 nulls last
    @Test
    public void sort() {
